...
<img alt="physical schema" src="https://github.com/marcussss1/Telegram/blob/main/images/physic.jpg">
...
Для хранения информации о сессиях, пользователей в чатах, сообщений в чатах и фотографий в чатах используем базу данных Redis по причине того, что она осуществляет хранение данных in-memory, имеет поддержку неблокирующей репликации master-slave и возможность организации кластера Redis cluster.
...
Рассмотрим алгоритм доставки сообщения, разобрав путь одного сообщения:

   На уровне nginx'а приходит запрос, там соединение апгрейдится до вебсокета и идет в messages-service, на уровне messages-service мы получаем сообщение по вебсокету, проверяем имеет ли пользователь доступ к этому чату, формируем структурки, которые определяют получателя сообщения и асинхронно пишем в бд.

   Теперь messages-service передает эти структурки для получателей в producer-service - его основная функциональность класть сообщения в kafka, также реализован dead-letter-exchange. 

   Далее kafka - это брокер сообщений, который маршрутизирует собщения, обладает надежностью и отказоустойчивостью, даже если kafka упадет, то не потеряет сообщения, т.к. он персистентный. 

   Дальше идет consumer-service он достает сообщения из очереди и публикует их в centrifugo, важно заметить, что сообщение будет помечено взятым из очереди, только тогда, когда от centrifugo мы получим ок, если consumer упадет, то как только встанет достанет все сообщения из очереди и опубликует в centrifugo, также тут мы ходим в dead-letter-exchange, в которой лежат не отправленные сообщения и публикуем в centrifugo.

   Centrifugo нужна для следующего: так как микросервисы у нас общаются по grpc, ходить в вечном цикле в consumer-service не совсем хорошая идея, поэтому будем использовать centrifugo как шину данных, к которой мы подключаемся по вебсокету из consumer-service и messages-service, также она является персистентной, если centrifugo упадет, то как только встанет восстановит все соединения и подписчики получат сообщения.

   Наконец messages-service, подписанный на события в centrifugo получает сообщение и оно отправляется получателю по вебсокету.
...
| Redis | Key-Value хранилище | In-memory БД позволяет обеспечить высокую скорость на чтение и запись. |
...
| Amazon S3 | Хранение файлов | Удобное апи для загрузки/отдачи файлов, скорость работы, масштабируемост. |
